...
from architectures import ModelC
...
def train_model_c():
    ...
    # We will simulate the UKF speed during training as the true speed 
    # plus some realistic random error (drift) to teach it to predict the residual.
    # In reality, we'd run the UKF to get the exact prior, but this is a standard approximation.
    ...
    for epoch in range(epochs):
        ...
        for X, ukf_v, target_dv in train_loader:
            optimizer.zero_grad()
            pred_dv = model(X, ukf_v)
            loss = criterion(pred_dv, target_dv)
            loss.backward()
            optimizer.step()
    ...
    torch.save(model.state_dict(), "models/stage3_velocity/weights/model_c.pt")
    print("Saved Model C.")
